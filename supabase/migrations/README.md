...
| `0014_settlement_import_idempotency.sql` | `dedup_key` + indice unico `(store_id, dedup_key)` en `settlement_rows` para que re-importar una liquidacion reemplace la fila por guia/orden | **Pendiente — REQUIERE 0010 antes** |
...
> **confirmarlo en prod**. `0014` (idempotencia de liquidaciones) es nueva y
