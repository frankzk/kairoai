...
| `0033_leads_stage_tuples.sql` | RPC `leads_stage_tuples`: agrupa en Postgres las cuatro columnas que deciden el bucket del tablero, para que los contadores salgan en UN viaje en vez de siete (PostgREST corta en 1.000 filas y en Costa Rica hay 6.212 leads elegibles). Devuelve las tuplas sin clasificar a proposito: el mapeo status -> bucket sigue viviendo solo en el catalogo de `lib/leads-classify.ts` | Si (30/08/2026) |
