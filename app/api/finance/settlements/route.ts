...
export async function POST(req: NextRequest) {
  try {
    ...
    const shopifyOrders = await fetchShopifyOrders(periodStart);
    const byName = new Map(shopifyOrders.map((order) => [order.name, order]));

    const statusSummary: Record<string, { count: number; amount_to_liquidate: number }> = {};
    let matchedRows = 0;

    const pendingRows = settlementRows.map((row) => {
      const shopify = byName.get(row.order_name);
...
async function fetchShopifyOrders(periodStart: string | null): Promise<ShopifySettlementOrder[]> {
  ...
  const minDate = periodStart
    ? new Date(new Date(periodStart).getTime() - 90 * 24 * 60 * 60 * 1000).toISOString()
...
function buildSettlementRow(
  ...
  return {
    ...
    internal_status: shopify ? row.internal_status : "unmatched",
