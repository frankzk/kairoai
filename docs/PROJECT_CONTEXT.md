...
- Shopify historical sync must be done via `/api/finance/shopify-sync` in bounded batches. The finance UI treats Shopify as the complete order base from `2026-01-01T00:00:00-06:00`; Boxful logistics and liquidations only enrich tracking/cash state. Page load reads persisted Shopify orders in small paginated API calls (`limit` + keyset `cursor`) so January onward can be shown without one oversized Vercel response.
...
- `/api/finance/shopify-sync` GET pages the persisted Shopify base with `limit` and a keyset `cursor` (last `id` seen, returned as `next_cursor`), so `/admin/finance` can load the full base without one large JSON response. Keyset pagination replaced deep `OFFSET` reads: an `OFFSET` of thousands forced Postgres to sort the whole table (dragging each row's JSONB) and discard most rows per page, which tripped the statement timeout and surfaced as `FUNCTION_INVOCATION_TIMEOUT`. The total count (`coverage`) is now computed only on the first page.
